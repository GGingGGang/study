...
> ★★★ **핵심**: 서비스 메시는 이 횡단 관심사를 **애플리케이션 바깥의 프록시 계층**으로 옮긴다. 개발자는 비즈니스 로직만 작성하고, mTLS·재시도·트래픽 분할·메트릭은 메시가 **코드 변경 없이** 제공한다. "라이브러리가 아니라 인프라로 해결한다"가 한 문장 요지다.
...
**istiod** 는 Istio의 컨트롤플레인을 하나의 바이너리로 묶은 컴포넌트다(과거 Pilot/Citadel/Galley가 통합됨). 주요 역할 세 가지를 기억하면 된다.
...
**Envoy** 는 C++로 작성된 고성능 L7 프록시로, Istio 데이터플레인의 핵심 엔진이다. 동적 설정을 받기 위한 표준 API가 **xDS**다.
...
**HBONE(HTTP-Based Overlay Network Environment)** 은 ztunnel이 노드 간 트래픽을 실어 나르는 터널 방식이다. **mTLS로 보호된 HTTP/2 CONNECT** 위에 원래 트래픽을 캡슐화해 보낸다. 즉 ztunnel끼리는 HBONE 터널을 열어 L4 트래픽을 암호화·식별하며 전달한다.
...
> ★ Gateway 리소스에는 두 가지 계보가 있다. 전통적 Istio **Gateway** CRD와, 쿠버네티스 표준인 **Gateway API**(Gateway/HTTPRoute 등)다. 우리 프로젝트는 외부 노출에 **Gateway API**를 쓰며, 그 트래픽이 OCI NLB를 거쳐 `ggang.cloud` 로 들어온다. 이 둘은 같은 데이터플레인을 설정하는 서로 다른 선언 방식이다.
