...
**서비스 메시(service mesh)** 는 마이크로서비스 사이의 **서비스-투-서비스 통신**을 처리하는 전용 인프라 계층이다. 핵심 아이디어는 단순하다. 트래픽 제어·보안·관측 같은 **횡단 관심사(cross-cutting concern)** 를 애플리케이션 코드에서 떼어내 플랫폼이 일괄로 책임진다.
...
이걸 서비스마다 라이브러리로 직접 구현하면 ① 언어/프레임워크마다 다시 짜야 하고(polyglot 문제) ② 버전이 제각각으로 갈리고 ③ 비즈니스 로직과 인프라 로직이 뒤섞인다.
...
서비스 메시는 SDN과 마찬가지로 **control plane / data plane 분리** 철학을 따른다.
...
**istiod** 는 Istio의 컨트롤플레인을 하나의 바이너리로 묶은 컴포넌트다(과거 Pilot/Citadel/Galley가 합쳐졌다). 주요 역할은 세 가지다.
...
**Envoy** 는 C++로 작성한 고성능 L7 프록시로, Istio 데이터플레인의 핵심 엔진이다. 동적 설정은 **xDS**라는 표준 API로 받는다.
...
Istio가 제공하는 데이터플레인 모드는 두 가지다. 우리 프로젝트는 **Ambient 모드**를 쓴다.
...
**HBONE(HTTP-Based Overlay Network Environment)** 은 ztunnel이 노드 간 트래픽을 실어 나르는 터널 방식이다. **mTLS로 보호된 HTTP/2 CONNECT** 위에 원래 트래픽을 캡슐화해 보낸다. ztunnel끼리 HBONE 터널을 열어 L4 트래픽을 암호화·식별하며 전달하는 구조다.

## 5. xDS — 설정 전달 흐름

**xDS** 는 컨트롤플레인이 데이터플레인 프록시를 동적으로 설정하는 API 집합이다(Envoy 표준). 주요 서브-API는 다음과 같다.
...
Istio는 쿠버네티스 CRD(Custom Resource Definition)로 정책을 선언한다. 자세한 사용법은 트래픽/보안 문서로 미루고, 여기서는 전체 지도를 잡는다.
