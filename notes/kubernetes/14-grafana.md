...
흩어진 지표를 각 도구의 CLI로 보는 대신, Grafana로 **하나의 대시보드에서 메트릭·로그·트레이스를 교차로** 본다(예: 그래프에서 스파이크를 클릭 → 그 시간대 로그로 점프).
...
★ 데이터소스끼리 **연계**가 가능하다. 예: Loki 로그의 `trace_id`에서 Tempo 트레이스로 점프, 메트릭 그래프의 한 지점에서 같은 라벨의 로그로 이동(데이터 링크/derived fields). 이게 "한 화면에서 메트릭→로그→트레이스로 파고드는" 옵저버빌리티의 핵심 흐름이다.
...
대시보드를 **재사용 가능하게** 만드는 핵심 기능이다. 하드코딩된 값 대신 **변수**를 두고, 화면 위 드롭다운으로 바꾼다. ★★★
...
- 변수 값은 **데이터소스에 질의해 자동으로 채울 수 있다**(예: `label_values(http_requests_total, namespace)` → 현재 존재하는 네임스페이스 목록).
...
> 참고: 메트릭 알럿은 Prometheus의 Alertmanager로도 처리할 수 있다. Grafana 알럿팅은 **여러 데이터소스(로그·DB 포함)에 걸쳐** 룰을 만들 수 있다는 점이 다르다.

## 6. 프로비저닝 (코드로 관리)

대시보드·데이터소스·알럿을 **UI에서 손으로 만들면 재현·버전관리가 안 된다.** 프로비저닝은 이를 **YAML/JSON 파일로 선언해 Grafana 시작 시 자동 적용**하는 방식이다(GitOps 친화적). ★★★
