...
GitOps의 원리(선언형·단일 진실 소스·자동 pull·지속 조정)는 정해져 있다. 실무의 난점은 **"그 선언들을 Git에 어떻게 배치·구조화·자동화할 것인가"** 다. 이 문서는 그 **구현 패턴**을 다룬다. 잘못 설계하면 GitOps가 오히려 더 복잡해지니, 패턴 선택의 **이유와 함정**까지 같이 본다.
...
- **무한 루프 방지**: 통합 repo에서 CI가 이미지 태그를 같은 repo에 커밋하면, 그 커밋이 다시 CI를 트리거하는 루프가 생기기 쉽다. config repo를 분리하면 깔끔하다.
...
정답은 없다. **조직 규모와 권한 경계**로 결정한다. 작게 시작할 땐 모노레포, 팀이 늘면 분리하는 식이 흔하다.
...
**app-of-apps**는 **"애플리케이션들을 선언하는 또 하나의 애플리케이션"** 을 두는 패턴이다. 부모 Application 하나가 여러 자식 Application 정의를 Git에서 읽어 일괄 생성·관리한다.
...
함정: 부모가 자식의 **prune(삭제) 권한**까지 쥐게 되니, root에서 자식 정의를 잘못 지우면 줄줄이 삭제될 수 있다. prune 정책을 신중히.
...
동기화 **특정 시점에 한 번 실행되는 작업**(보통 Job). 선언형으로 표현하기 어려운 절차적 작업을 끼워넣는 장치다.
...
- "롤백"조차 하나의 **추적 가능한 커밋**으로 남아 감사 일관성을 유지한다.
- 단, **시크릿/외부 상태**(DB 스키마 등)는 Git revert만으로 안 돌아갈 수 있다. 마이그레이션 역적용 등 별도로 고려해야 한다.

---

## 6. 이미지 업데이트 자동화

CI가 새 이미지를 만들면, **그 새 태그를 config repo에 반영**해야 CD가 배포한다. 이 "이미지 태그 → Git 커밋" 단계의 자동화가 GitOps의 핵심 자동화 포인트다.
...
GitOps의 "버전·불변" 정신상 **다이제스트(sha256) 고정**을 권장한다. 최소한 `latest` 같은 떠다니는 태그는 피하고, **불변 태그(빌드별 고유 태그)** 나 다이제스트를 쓴다.
...
**평문 시크릿을 Git에 커밋하면 안 된다.** Git 이력은 영구라 한 번 들어간 비밀은 사실상 회수하지 못한다. GitOps에서 시크릿을 다루는 대표 접근(개념):
...
새 버전을 한 번에 전부 교체하지 않고 **점진적으로 트래픽을 옮기며** 위험을 줄이는 기법. GitOps와 묶어 배포 전략 자체를 선언형으로 관리한다.
