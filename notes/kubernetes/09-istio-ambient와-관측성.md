...
이 문서는 우리 프로젝트가 채택한 **Istio Ambient 모드**의 동작을 깊게 보고, Istio가 **무엇을 관측 데이터로 내보내는지**에 집중한다. 관측성 백엔드 도구의 사용법은 다루지 않는다(Istio가 표준 포맷으로 데이터를 내보내면, 어떤 호환 백엔드든 받을 수 있다는 관점). 사이드카 모드는 비교용으로만 언급한다. 외부 진입은 Gateway API + OCI NLB로 `ggang.cloud` 에 연결된다.
...
**ztunnel(zero-trust tunnel)** 은 각 노드에서 도는 **Rust 기반 경량 데몬**(DaemonSet)이다.
...
- **L7은 안 함**: HTTP 라우팅·리트라이·경로 기반 인가 같은 건 ztunnel이 하지 않는다. 그건 waypoint의 몫.
...
**waypoint** 는 L7 기능이 필요한 **네임스페이스 또는 서비스 단위로 배치하는 Envoy 기반 프록시**다(Deployment).
...
서비스 메시의 큰 가치 중 하나는 **코드 변경 없이 균일한 텔레메트리**를 얻는 것이다. 데이터플레인이 모든 트래픽을 보므로, **메트릭·트레이싱·로그**를 자동 생성해 표준 포맷으로 내보낸다. 우리는 "Istio가 무엇을 내보내는가"에 집중한다(특정 백엔드 도구는 논외).
